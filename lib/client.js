// ============================================================================
// 按键切换模型 —— 客户端插件（全局 bundle 版，浏览器端）
// ----------------------------------------------------------------------------
// 从 .agent-presets/model-switch 迁移而来。改这个文件后：
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      if (pickerFor === 'learn') {
        return h('div', { className: 'rms-panel', style: panelStyle() }, [
          ...
          h('div', { style: { color: '#555', lineHeight: 1.7 } }, '按下要绑定的按键（建议 F13–F24 或 Ctrl/Alt 组合）。'),
      ...
      return h('div', { className: 'rms-panel', style: panelStyle() }, [
        closeBtn,
        h('div', { style: { fontWeight: 600, marginBottom: 8 } }, '按键切换模型'),
        ...
        keys.length === 0
          ? h('div', { style: { color: '#888', padding: '6px 0' } }, '还没有映射。点击“添加按键映射”，然后按下要绑定的键（建议 F13–F24 或 Ctrl/Alt 组合）。')
          : keys.map((key) => {
              const m = mappings[key]
        ...
        const stop = ctx.slots.inject(name, () => {
          try {
            const d = ctx.slots.register(
              { name, id, order, label: function () { return '按键切换模型' } },
              component,
            )
    ...
    const btnOk = mountSeat('conversation.input.left', 'model-switch', 0, RemoteModelSwitchButton)
    const panelOk = mountSeat('shell.overlay', 'model-switch-panel', 1000, RemoteModelSwitchPanel)

// dsh-remote-model-switch — 宿主半。
// 1) 启动标记：证明本行被宿主挂载（$DSH_HOME/.rms-host-loaded.json）。
// 2) 诊断路由：浏览器半把加载/激活状态 POST 到 /remote-model-switch/diag，
...
export const name = 'remote-model-switch'
export const inject = ['webServer']

const HOME = process.env.DSH_HOME || process.cwd()
const MARKER = join(HOME, '.rms-host-loaded.json')
const DIAG = join(HOME, '.rms-diag.ndjson')
...
export function apply(ctx) {
  ...
  ctx.effect(() => {
    ...
    const postRoute = ctx.webServer.register({ kind: 'exact', path: '/remote-model-switch/diag', handler: post })
    const getRoute = ctx.webServer.register({ kind: 'exact', path: '/remote-model-switch/state', handler: get })
    return () => { postRoute(); getRoute() }
  }, 'dsh-remote-model-switch: diag routes')
}
